...
export default async function Recipe(props: RecipeProps) {
  ...
  let img = <></>;
  if(meta.img != null) {
    const imgModule = (await import(`/siteContent/recipes/${props.params.slug}/${meta.img}`)).default;
    img = <Link href={imgModule.src}><Image src={imgModule} alt={meta.Title} className="m-auto w-[80%]" /></Link>
  }
